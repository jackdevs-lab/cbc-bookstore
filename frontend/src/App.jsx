...
const CBCEcommerce = () => {
  ...
  // Fixed: Correct filtering logic with proper state handling
  const filteredProducts = products
    .filter(p => {
      const matchSearch = p.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
                         (p.isbn && p.isbn.includes(searchQuery)) ||
                         (p.publisher && p.publisher.toLowerCase().includes(searchQuery.toLowerCase()));
      
      const matchGrade = filters.grades.length === 0 || 
                        (p.grade_id && filters.grades.includes(p.grade_id));
      
      const matchSubject = filters.subjects.length === 0 || 
                          (p.subject_id && filters.subjects.includes(p.subject_id));
  ...
  // Fixed Navigation Helpers - This is the key fix!
  ...
  // DetailsPage, CartModal, CheckoutPage, SuccessPage remain unchanged
  // (They were already working correctly)
